import { getAuthApiHeaders, validateCsrfToken, checkRateLimit } from './_security-utils.js';
...
export async function onRequestPost({ request, env }) {
  ...
  if (!validateCsrfToken(request)) {
    return new Response(
      JSON.stringify({ ok: false, error: 'csrf_failed' }), 
      { status: 403, headers }
    );
  }
  ...
  if (action === 'login' || action === 'register') {
    const rateLimitResult = await checkRateLimit(request, env, 'auth', 5, 3600000);
  ...
  if (!/^[a-zA-Z0-9_-]+$/.test(username)) {
    return new Response(JSON.stringify({ ok: false, error: 'invalid_username_format' }), { status: 400, headers });
  }

  // Only check password field for register/login actions
  if ((action === 'register' || action === 'login') && !password) {
    return new Response(JSON.stringify({ ok: false, error: 'bad_password' }), { status: 400, headers });
  }
  ...
  if (action === 'register') {
    ...
    if (exists?.id) {
      return new Response(JSON.stringify({ ok: false, error: 'username_taken' }), { status: 409, headers });
    }
    ...
    responseHeaders.append('Set-Cookie', setCookieHeader(username));
  ...
  if (action === 'login') {
    ...
    if (!isValid) {
      return new Response(JSON.stringify({ ok: false, error: 'bad_credentials' }), { status: 401, headers });
    }
    ...
    return new Response(JSON.stringify({ ok: true, me }), { headers: responseHeaders });
  }
